...
import { fetchAllBranchList, fetchCustomeSegment } from '../../service/api_services';

const { Option } = Select;

const SelectedSegment = ({ setSegmentId, isFilter }) => {
    const { token } = useAuth();
    ...
    const fetchData = async (page) => {
        ...
        setFetching(true);
        try {
            const res = await fetchCustomeSegment(token);
            console.log(res, "custom segment")
    ...
    return (
        <Form.Item
            ...
            name="staticCustomSegmentId"
            rules={[
                {
                    required: true,
                    message: 'Please Select Custom Segment!',
                },
            ]}
        ...
        >
            ...
            >
                {data.map((item) => (
                    <Option key={item.segmentId} value={item.segmentId}>
