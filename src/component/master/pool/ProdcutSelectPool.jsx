...
const ProdcutSelectPool = ({ setXlsFile, setSelectedPool }) => {
    const { token } = useAuth();
    ...
    const handleSelectChange = (values) => {
        const transformed = values.map((productCode) => {
            const option = data.find((opt) => opt.productCode === productCode);
            return option ? { productCode: option.productCode, categoryID: option.categoryID, } : null;
        }).filter(Boolean); // Filter out null values (in case of no match)
...
export default ProdcutSelectPool