...
const BrandSelect = ({ ShowAllProductsList, setNavigateName, brandId }) => {
    const { token } = useAuth();
    ...
    const handleChange = (value) => {
        ...
        const catObj = data.find(item => item.brandID === value);
        console.log("catObj:", catObj);
        if (catObj) {
            setNavigateName(catObj.brandName);
        } else {
    ...
    return (
        ...
        >
            ...
            >
                {data.map((item) => (
                    <Option key={item.brandID} value={item.brandID} label={item.brandName}>
                        {item.brandName}
                    </Option>
                ))}
            </Select>
