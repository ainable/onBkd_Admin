...
const CategorySelect = ({ setNavigateName }) => {
    const { token } = useAuth();
    ...
    const fetchData = async (page) => {
        setFetching(true);
        try {
            const res = await FetchAllCategoryList(token, page);
    ...
    const handleChange = (value) => {
        ...
        const catObj = data.find(item => item.categoryID === value);
        console.log("catObj:", catObj);
        if (catObj) {
            setNavigateName(catObj.categoryName);
        } else {
    ...
    return (
        ...
        >
            ...
            >
                {data.map((item) => (
                    <Option key={item.categoryID} value={item.categoryID} label={item.categoryName}>
                        {item.categoryName}
                    </Option>
                ))}
            </Select>
